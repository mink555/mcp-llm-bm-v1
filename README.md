...
| [모델별 특성 및 주요 이슈](#모델별-특성-및-주요-이슈) | 각 모델의 강점/약점 및 발견된 문제 |
...
| 모델명 | 강점 | 약점 | 종합 평가 |
|:---|:---|:---|:---|
| **Qwen3-32B** | SingleCall 85.8%로 압도적 1위 | CallDecision 31.7%로 보통 | Tool-use 특화 튜닝이 잘 되어 있음. 가성비 최고 |
| **Qwen3-Next-80B** | CallDecision 32.7%로 최고 | 80B임에도 32B보다 낮은 종합 점수 | 호출 판단은 좋으나 실제 호출 정확도는 32B가 우수 |
| **Qwen3-14B** | 소형 모델 대비 SingleCall 80.6% 준수 | Dialog 42.0%로 다소 낮음 | 경량화 환경에서 추천할 만한 성능 |
| **Llama-3.3-70B** | Dialog 48.0%로 전체 1위 | CallDecision 10.7%로 최하위 | 대화 능력은 우수하나 Tool-use 특화 튜닝 부재 |
| **Mistral-Small-24B** | Dialog 47.0%로 준수 | SingleCall 16.0%로 최하위 | 함수 선택 능력이 매우 약함 |

### 평가 중 발견된 주요 이슈
| 모델 | 이슈 유형 | 상세 내용 | 영향 |
|:---|:---|:---|:---|
| **Llama-3.3-70B** | CallDecision 취약 | 함수 호출이 필요한 상황에서 자연어로만 응답하거나, 불필요한 상황에서 억지로 호출하는 경향 | CallDecision 정확도 10.7% (최하위) |
| **Mistral-Small-24B** | 함수 선택 오류 | 유사 함수 중 정답을 찾지 못하고 다른 함수를 선택하는 비율이 높음 | SingleCall 정확도 16.0% (최하위) |
| **Mistral-Small-24B** | tool_call_id 형식 | 표준 형식과 다른 ID를 생성하여 파싱 오류 발생 | 코드 레벨에서 9자리 영숫자로 normalize 처리 |
| **Qwen 시리즈** | 모델 크기 역전 | 80B 모델이 32B보다 낮은 점수 기록 | 모델 크기보다 Tool-use 특화 튜닝 버전이 중요 |

### 용도별 모델 추천
| 용도 | 추천 모델 | 이유 |
|:---|:---|:---|
| **Agent 역할 (Tool 호출 중심)** | Qwen3-32B | SingleCall 85.8%로 함수 선택 정확도 최고 |
| **챗봇 답변 (대화 품질 중심)** | Llama-3.3-70B | Dialog 48.0%로 자연스러운 대화 능력 최고 |
| **경량화 환경** | Qwen3-14B | 14B 크기에서 80% 이상의 함수 선택 정확도 |
| **균형 잡힌 성능** | Qwen3-Next-80B | 모든 카테고리에서 고르게 준수한 성능 |
